...
// Nº de contrato/processo cadastrado para a cidade do usuário (aba Cidades).
// Vai no /api/me só pra permitir que o relatório de medição do próprio
// encarregado mostre o contrato certo da cidade dele — ele continua sem
// acesso à aba Cidades nem aos documentos de contrato/aditivo em si.
function contratoDoUsuario(u) {
  if (!u || !u.cidadeId) return null;
  const c = state.cidades.find((x) => x.id === u.cidadeId);
  return c && c.contrato ? c.contrato : null;
}
...
function usuarioPublico(u) {
  ...
  return {
    ...
    cidadeContrato: u.role === "admin" ? null : contratoDoUsuario(u),
  };
}
