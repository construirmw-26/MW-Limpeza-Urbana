  ...
  try {
    ...
    if (urlPath.startsWith("/api/")) {
      ...
      if (servicoMatch && method === "DELETE") {
        ...
        if (alvo) delete state.metas[alvo.nome]; // remove também a meta associada a esse serviço
        persist();
        return sendJSON(res, 200, state.servicos);
      }

      // ---- Metas ----
      // Qualquer um logado pode VER o progresso; só o administrador define
      // o valor da meta.
      if (urlPath === "/api/metas") {
        if (method === "GET") return sendJSON(res, 200, state.metas);
        if (method === "PUT") {
          if (!requireAdmin(req, res)) return;
          const body = await readJSON(req);
          ...
          if (valor > 0) state.metas[servico] = valor;
          else delete state.metas[servico];
          persist();
          return sendJSON(res, 200, state.metas);
        }
      }
